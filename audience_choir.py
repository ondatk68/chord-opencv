...
def draw_rec_l(image):
    ...
    if(l_c_r_l=='l'):
        ...
        cv2.rectangle(image,(int(width/3),0), (int(width*2/3),height), (255,0,0),10)
    else:
...
def draw_rec_r(image):
    ...
    if(l_c_r_r=='l'):
        ...
        cv2.rectangle(image,(int(width/3),0), (int(width*2/3),height), (0,255,0),10)
    else:
...
def man_audio():
    ...
    def find_outliers(input_array):
        ...
        return input_array[( q1 - 1.5 * iqr <= input_array ) & (input_array <= q3 + 1.5 * iqr )]
    df = find_outliers(df)
    print(df.mean())
    f0 = df.mean()
    ...
    sample = [ np.sin( np.arange(44100 * per_length) * pitch[pitch_idx[0]%12] * 2 ** int(pitch_idx[0]//12) * np.pi * 2 / 44100), 
           np.sin(np.arange(44100 * per_length) * pitch[pitch_idx[1]%12] * 2 ** int(pitch_idx[1]//12) * np.pi * 2 / 44100), 
           np.sin(np.arange(44100 * per_length) * pitch[pitch_idx[2]%12] * 2 ** int(pitch_idx[2]//12) * np.pi * 2 / 44100)]
    ...
    while(1):
        for  i in range(3):
            if dy[i] >= 0:
                sample[i] = np.sin(np.arange(44100 * per_length) * pitch[pitch_idx[i]%12] * 2 ** int(pitch_idx[i]//12) * np.pi * 2 / 44100 + last[i]) 
            else:
                sample[i] = np.sin(np.arange(44100 * per_length) * pitch[pitch_idx[i]%12] * 2 ** int(pitch_idx[i]//12) * np.pi * 2 / 44100 + np.pi - last[i]) 
        ...
        stream.write(((sample[0] + sample[1] + sample[2]) / 3).astype(np.float32).tostring())
...
def showVideo():
    ...
    while cap.isOpened():
        ...
        if preset_count > 0:
            cv2.putText(image, text=str(int(preset_count//fps + 1)), org=(int(width//2), int(height//2)), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale = 2.0, color = (0,0,255), thickness = 5, lineType=cv2.LINE_8) 
        else:
            rec_st = 1
            cv2.putText(image, text="recording...", org=(int(width//2)-70, int(height//2)), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale = 1.0, color = (0,0,255), thickness = 5, lineType=cv2.LINE_8) 
            cv2.rectangle(image,(0,0), (640,480), (0,0,255),10)
    ...
    processing[:,:,2] = 255
    cv2.putText(processing, text="processing...", org=(int(width//2)-70, int(height//2)), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale = 1.0, color = (255,255,255), thickness = 3, lineType=cv2.LINE_8) 
    while(1):
    ...
    root_disp[:,:,2] = 255
    cv2.putText(root_disp, text="root is " + tone[root_idx], org=(int(width//2)-50, int(height//2)), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale = 1.0, color = (255,255,255), thickness = 3, lineType=cv2.LINE_8) 
    ...
    org_lcr = [(int(width/6), int(height/2)), (int(width*3/6), int(height/2)), (int(width*5/6), int(height/2))]
        ...
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks: #手が検知されているとき
                if(len(results.multi_hand_landmarks)==1): #手が1個のとき
                    if(results.multi_handedness[0].classification[0].label=='Left'): #左手のみ
                        detected=[1,0]
                        prev_index_tip_l = index_tip_l
                        index_tip_l = [results.multi_hand_landmarks[0].landmark[8].x, results.multi_hand_landmarks[0].landmark[8].y]
                        if check_is_gu(results.multi_hand_landmarks[0].landmark):
                            is_gu_l += 1
                        else:
                            is_gu_l = 0

                    elif(results.multi_handedness[0].classification[0].label=='Right'): #右手のみ
                        detected=[0,1]
                        prev_index_tip_r = index_tip_r
                        index_tip_r = [results.multi_hand_landmarks[0].landmark[8].x, results.multi_hand_landmarks[0].landmark[8].y]
                        if check_is_gu(results.multi_hand_landmarks[0].landmark):
                            is_gu_r += 1
                        else:
                            is_gu_r = 0
                ...
                if(len(results.multi_hand_landmarks)==2):
                    if(results.multi_handedness[0].classification[0].label == results.multi_handedness[1].classification[0].label):
                        ...
                        if(results.multi_handedness[0].classification[0].label=='Left'): #左手→右手
                            prev_index_tip_l = index_tip_l
                            index_tip_l = [results.multi_hand_landmarks[0].landmark[8].x, results.multi_hand_landmarks[0].landmark[8].y]
                            if check_is_gu(results.multi_hand_landmarks[0].landmark):
                                is_gu_l += 1
                            ...
                            prev_index_tip_r = index_tip_r
                            index_tip_r = [results.multi_hand_landmarks[1].landmark[8].x, results.multi_hand_landmarks[1].landmark[8].y]
                            if check_is_gu(results.multi_hand_landmarks[1].landmark):
                                is_gu_r += 1
                            else:
                                is_gu_r = 0
                        elif(results.multi_handedness[1].classification[0].label=='Left'): #右手→左手
                            prev_index_tip_l = index_tip_l
                            index_tip_l = [results.multi_hand_landmarks[1].landmark[8].x, results.multi_hand_landmarks[1].landmark[8].y]
                            if check_is_gu(results.multi_hand_landmarks[1].landmark):
                                is_gu_l += 1
                            else:
                                is_gu_l = 0
                ...
                if(detected[0]):
                    ...
                    cv2.circle(image, (int(index_tip_l[0]*width), int(index_tip_l[1]*height)), 30,(255,0,0),-1)
                ...
                if(detected[1]):
                    ...
                    cv2.circle(image, (int(index_tip_r[0]*width), int(index_tip_r[1]*height)), 30,(0,255,0),-1)
            ...
            for i in range(3):
                if updown[i] == 1:
                    ...
                    print('up')
                if up_st[i] != -1:
                    cv2.putText(image, text='up', org=org_lcr[i], fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale = 1.0, color = (0,0,255), thickness = 5, lineType=cv2.LINE_8) 
                    up_st[i] += 1
                    if up_st[i] == fps:
                        up_st[i] = -1
                if updown[i] == -1:
                    ...
                    print('down')
                if down_st[i] != -1:
                    cv2.putText(image, text='down', org=org_lcr[i], fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale = 1.0, color = (0,0,255), thickness = 5, lineType=cv2.LINE_8)
                    down_st[i] += 1
                    if down_st[i] == fps:
                        down_st[i] = -1
                dia_idx[i] += updown[i]
            ...
            for i in range(3):
                if(is_gu[i]):
                    cv2.putText(image, text="rest", org=(org_lcr[i][0], int(org_lcr[i][1]/2)), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale = 1.0, color = (15,192,252), thickness = 5, lineType=cv2.LINE_8)
                else:
                    cv2.putText(image, text=tone[pitch_idx[i]%12], org=(org_lcr[i][0], int(org_lcr[i][1]/2)), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale = 1.0, color = (15,192,252), thickness = 5, lineType=cv2.LINE_8)
